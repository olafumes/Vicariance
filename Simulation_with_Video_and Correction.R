...
Correction_Factor <- vector(length = 100)
...
makeplot <- function(){ #function for video
	for (m in 1:500){
		...
		Spore_Arrival_A_Sum <- apply(Spore_Arrival_A,1, sum)#at each locality some the arrived spores
		Spore_Arrival_B_Sum <- apply(Spore_Arrival_B,1, sum)
		Spore_Arrival_Total <- Spore_Arrival_A_Sum + Spore_Arrival_B_Sum
		Growth_Rate <- ifelse(Spore_Arrival_Total > 1, 1/Spore_Arrival_Total, 1 + (r * Spore_Arrival_Total - r * Spore_Arrival_Total * Spore_Arrival_Total)) #if spore arrival >1 adjust to one in order not to pass capacity, else allow logistic growth 
		...
		propagation_B <- Selection_Vector_B * Species_B_Distribution * Correction_Factor
...
for (m in 1:500){ #as above for graphics production
		for (i in 1:100){
			for (j in 1:100){
				Spore_Arrival_A[i,j] <- Dispersion[abs(i-j)+1]*propagation_A[j]
				Spore_Arrival_B[i,j] <- Dispersion[abs(i-j)+1]*propagation_B[j]
